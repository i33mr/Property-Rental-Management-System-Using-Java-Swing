...
// after clicking view more details in property manager home page, this panel will show the property details
public class SpecificPropertyView extends JPanel{
	private int currentPicIndex = 0; // index of picture path in the array of paths
	private JScrollPane scrollPane; // main scroll pane
	private JPanel mainPanel;	// panel inside the scrollpane, which contains all other panels
	private JPanel picSlider; // pictures panel
	private JLabel leftArrow; // picSlider left button
	private JLabel rightArrow; // picSlider right button
	private JLabel shownPictureLabel; // label that holds the property picture as its icon
	...
	private JLabel appNameLabel; // CYBERSPACE label
	private JTextPane propertyDescriptionTextPane; // property description
	private JScrollPane descriptionPanel; // scroll pane for the description
	private JPanel facilitiesPanel; // panel holds all property facilities inside it
	private JLabel facilitiesHeaderLabel;
	private JSeparator facilitiesSeparator;
	private JPanel featuresPanel; // panel holds all property features inside it
	private JLabel featuresHeaderLabel;
	private JSeparator featuresSeparator;
	private JPanel applyToRentPanel; // panel holds rent application components
	private JLabel rentRequestLabel;
	private JButton rentRequestButton; // aply to rent
	private JPanel managerInfoPanel; // shows property manager info after clicking the application button
	...
	private Property currentProperty; // the property that currently in the view
	private JLabel rentRateLabel;
	private User propertyManager; // property manager of the current property
	
	public SpecificPropertyView() {
		initialize();
		initializePicSlider();
	...
	public SpecificPropertyView(Property currentProperty) {
	...
	// initialize the picture slider panel
	...
	// initialize property info using currentProperty
	...
	// initialize the header (back label, app name)
	...
	// initialize facilities panel, get their icons from the files
	...
	// initialize features panel, get their icons from the files
	...
	// initialize rent application panel
	private void initializeApplyToRent() {
		...
		rentRequestLabel = new JLabel("Interested in this property?");			
		rentRequestLabel.setBounds(93, 10, 447, 40);
		rentRequestLabel.setFont(new Font("Source Sans Pro Black", Font.PLAIN, 20));
		applyToRentPanel.add(rentRequestLabel);
		
		rentRequestButton = new JButton("Apply to Rent it Now!");
			
		rentRequestButton.setBackground(new Color(255,240,149));
		rentRequestButton.setFocusable(false);
		rentRequestButton.setBounds(550, 5, 400, 40);
		rentRequestButton.setFont(new Font("Source Serif Pro Black", Font.PLAIN, 20));
		applyToRentPanel.add(rentRequestButton);
		...
		try {
			readPropertyManagerFromFile(); // set user object from files
		...
		if(currentProperty.getPropertyActivationStatus().equals("Deactivated")) {
			rentRequestLabel.setText("This property is deactivated by  its manager");
			rentRequestButton.setEnabled(false);
		}
	...
	public JButton getRentRequestButton() {
		return rentRequestButton;
	}
